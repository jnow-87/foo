...
/* macros */
#define KMSG_ANY	(FATAL | WARN | DEBUG | INFO | STAT)

#ifdef CONFIG_KERNEL_MSG_FATAL
#define	FATAL(fmt, ...)		cprintf(FATAL, FG_RED "[FATAL]" RESET_ATTR " %25s()\t" FG_RED fmt RESET_ATTR, __FUNCTION__, ##__VA_ARGS__);
#else
#define FATAL(fmt, ...)
#endif // CONFIG_KERNEL_MSG_FATAL

#ifdef CONFIG_KERNEL_MSG_WARN
#define WARN(fmt, ...)		cprintf(WARN, FG_YELLOW "[WARN]" RESET_ATTR " %25s()\t" FG_YELLOW fmt RESET_ATTR, __FUNCTION__, ##__VA_ARGS__);
#else
#define WARN(fmt, ...)
